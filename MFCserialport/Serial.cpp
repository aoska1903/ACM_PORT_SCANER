...
LONG CSerial::Open( INT nDevice, DWORD dwInQueue, DWORD dwOutQueue, BOOL fOverlapped )
{
	CString strDevice = _T("");
	strDevice.Format(_T("\\\\.\\COM%d"), nDevice);
	return Open(strDevice, dwInQueue, dwOutQueue, fOverlapped);
}
...
CSerial::EHandshake CSerial::GetHandshaking (void)
{
	...
	if ( dcb.fRtsControl == RTS_CONTROL_TOGGLE )
		return EHandshakeRtsToggle;
...
LONG CSerial::WriteBlock( LPCVOID pData, DWORD dwSize, DWORD dwTimeout )
{
	Purge();
	...
	{
		dwWritten = 0;
		if ( Write( lpData, dwLeft, &dwWritten, NULL, dwTimeout ) != ERROR_SUCCESS ) 
		{
			if ( m_lLastError == ERROR_TIMEOUT )
			{		
				return ERROR_NODATA;
			}		
			return ERROR_FAIL;
		...
		if ( ( dwWritten == 0 ) && ( GetTickCount() - dwStart >= TIMELIMIT ) )
		{	
			return ERROR_NODATA;
		}
...
LONG CSerial::ReadBlock( LPVOID pData, DWORD dwSize, DWORD dwTimeout )
{
	DWORD dwLeft, dwReaded = 0;
	DWORD dwStart = GetTickCount();

	BYTE *lpData = static_cast<BYTE *>(pData);
	dwLeft = dwSize;
	...
	while( dwLeft > 0 )
	{
		...
		if(  Read( lpData, dwLeft, &dwReaded, NULL, dwTimeout ) != ERROR_SUCCESS )
		{
			if ( m_lLastError == ERROR_TIMEOUT )
			{
				return ERROR_NODATA;
		...
		if( ( dwReaded == 0 ) && ( GetTickCount() - dwStart >= TIMELIMIT ) )
			return ERROR_FAIL;
...
BOOL CSerial::GetStatus( DWORD dwStatus )
{
	m_lLastError = ERROR_SUCCESS;
	...
	return ( dwModemStat & dwStatus ) != 0;
}

BOOL CSerial::GetCTS (void)
{
	return GetStatus( MS_CTS_ON );
}

BOOL CSerial::GetDSR (void)
{
	return GetStatus( MS_DSR_ON );
}

BOOL CSerial::GetRing (void)
{
    return GetStatus( MS_RING_ON );
}

BOOL CSerial::GetRLSD (void)
{
    return GetStatus( MS_RLSD_ON );
}


LONG CSerial::SetupReadTimeouts( DWORD dwInterval, DWORD dwMultiplier, DWORD dwConstant )
{
	m_lLastError = ERROR_SUCCESS;
	...
	cto.ReadIntervalTimeout         = dwInterval;
	cto.ReadTotalTimeoutMultiplier  = dwMultiplier;
	cto.ReadTotalTimeoutConstant    = dwConstant;
...
LONG CSerial::SetupWriteTimeouts( DWORD dwMultiplier, DWORD dwConstant )
{
	m_lLastError = ERROR_SUCCESS;
	...
	cto.WriteTotalTimeoutMultiplier = dwMultiplier;
	cto.WriteTotalTimeoutConstant   = dwConstant;
