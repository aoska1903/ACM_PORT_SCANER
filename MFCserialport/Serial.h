...
#else
#define SERIAL_DEFAULT_OVERLAPPED	FLASE
#endif
#endif

#define ERROR_FAIL		(-1)
#define ERROR_NODATA	(-2)
...
public:
	...
	typedef enum
	{
		...
		EHandshakeRtsToggle     =  3,   // the RTS line will be high if bytes are available for transmission. 
...
public:
	...
	// Open the serial communications for a particular COM port. 
	virtual LONG Open (LPCTSTR lpszDevice, DWORD dwInQueue = 0, DWORD dwOutQueue = 0, BOOL fOverlapped = SERIAL_DEFAULT_OVERLAPPED);	
	virtual LONG Open (INT     nDevice,    DWORD dwInQueue = 0, DWORD dwOutQueue = 0, BOOL fOverlapped = SERIAL_DEFAULT_OVERLAPPED);

	// Close the serial port.
	virtual LONG Close (void);

	BOOL CheckDeviceOpen();
	...
	virtual LONG WriteBlock( LPCVOID pData, DWORD dwSize, DWORD dwTimeout );
	...
	virtual LONG ReadBlock( LPVOID pData, DWORD dwSize, DWORD dwTimeout );
	// Send a break
	LONG Break (void);
	...
	//VOID SetLogFilePtr(CFileLog *_pLog){ m_log = *_pLog;}
	LONG SetupReadTimeouts( DWORD dwInterval, DWORD dwMultiplier, DWORD dwConstant );
	LONG SetupWriteTimeouts( DWORD dwMultiplier, DWORD dwConstant );
